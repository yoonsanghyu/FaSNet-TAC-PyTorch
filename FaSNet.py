...
class BF_module(DPRNN_base):
    ...
    def forward(self, input, num_mic):
        ...
        # pass to DPRNN
        if self.model_type == 'DPRNN':
            output = self.DPRNN(enc_segments).view(batch_size * ch * self.num_spk, self.feature_dim, self.segment_size,
                                                   -1)  # B*ch*nspk, N, L, K
        elif self.model_type == 'DPRNN_TAC':
            enc_segments = enc_segments.view(batch_size, ch, -1, enc_segments.shape[2],
                                             enc_segments.shape[3])  # B, ch, N, L, K
            output = self.DPRNN(enc_segments, num_mic).view(batch_size * ch * self.num_spk, self.feature_dim,
                                                            self.segment_size, -1)  # B*ch*nspk, N, L, K
        ...
        bf_filter = bf_filter.transpose(1, 2).contiguous().view(batch_size, ch, self.num_spk, -1,
                                                                self.output_dim)  # B, ch, nspk, L, N
...
class FaSNet_origin(FaSNet_base):
    ...
    def forward(self, input, num_mic):
        ...
        all_seg, all_mic_context, rest = self.seg_signal_context(input, self.window,
                                                                 self.context)  # B, nmic, L, win/chunk
        ...
        other_segment = all_seg[:, 1:].contiguous().transpose(0, 1).contiguous().view(nmic - 1, -1,
                                                                                      self.window)  # nmic-1, B*L, win
        ...
        if num_mic.max() == 0:
            ...
            ref_cos_sim = [ref_cos_sim[:num_mic[b], b, :].mean(0).unsqueeze(0) for b in
                           range(batch_size)]  # 1, L, 2*win+1
            ref_cos_sim = torch.cat(ref_cos_sim, 0).transpose(1, 2).contiguous()  # B, 2*win+1, L

        # pass to a DPRNN
        ref_feature = all_mic_context[:, 0].contiguous().view(batch_size * seq_length, 1,
        ...
        ref_filter = self.ref_BF(torch.cat([self.enc_LN(ref_feature), ref_cos_sim], 1),
                                 num_mic)  # B, 1, nspk, L, 2*win+1
        ...
        other_context_saved = other_context.view(batch_size * self.num_spk, nmic - 1, seq_length,
                                                 self.context * 2 + self.window)  # B*nspk, nmic-1, L, 3*win
        other_context = other_context_saved.transpose(0, 1).contiguous().view(nmic - 1, -1,
                                                                              self.context * 2 + self.window)  # nmic-1, B*nspk*L, 3*win
        ...
        other_cos_sim = other_cos_sim.view(nmic - 1, batch_size * self.num_spk, seq_length,
                                           self.filter_dim)  # nmic-1, B*nspk, L, 2*win+1
        other_cos_sim = other_cos_sim.permute(1, 0, 3, 2).contiguous().view(-1, self.filter_dim,
                                                                            seq_length)  # B*nspk*(nmic-1), 2*win+1, L

        # pass to another DPRNN
        other_feature = self.encoder(other_context_saved.view(-1, 1, self.context * 2 + self.window)).view(-1,
                                                                                                           seq_length,
                                                                                                           self.enc_dim)  # B*nspk*(nmic-1), L, N
        other_feature = other_feature.transpose(1, 2).contiguous()  # B*nspk*(nmic-1), N, L
        other_filter = self.other_BF(torch.cat([self.enc_LN(other_feature), other_cos_sim], 1),
                                     num_mic)  # B*nspk*(nmic-1), 1, 1, L, 2*win+1
        ...
        other_output = other_output.view(batch_size * self.num_spk, nmic - 1, seq_length,
                                         self.window)  # B*nspk, nmic-1, L, win
        ...
        bf_signal1 = bf_signal[:, :, :self.window].contiguous().view(batch_size * self.num_spk * nmic, 1, -1)[:, :,
                     self.stride:]
        bf_signal2 = bf_signal[:, :, self.window:].contiguous().view(batch_size * self.num_spk * nmic, 1, -1)[:, :,
                     :-self.stride]
...
class FaSNet_TAC(FaSNet_base):
    ...
    def forward(self, input, num_mic):
        ...
        enc_output = self.encoder(all_mic_context.view(-1, 1, self.context * 2 + self.window)).view(batch_size * nmic,
                                                                                                    seq_length,
                                                                                                    self.enc_dim).transpose(
            1, 2).contiguous()  # B*nmic, N, L
        ...
        all_context = all_mic_context.transpose(0, 1).contiguous().view(nmic, -1,
                                                                        self.context * 2 + self.window)  # 1, B*L, 3*win
        all_cos_sim = self.seq_cos_sim(all_context, ref_seg)  # nmic, B*L, 2*win+1
        all_cos_sim = all_cos_sim.view(nmic, batch_size, seq_length, self.filter_dim).permute(1, 0, 3,
                                                                                              2).contiguous()  # B, nmic, 2*win+1, L
        ...
        mic_context = torch.cat(
            [all_mic_context.view(batch_size * nmic, 1, seq_length, self.context * 2 + self.window)] * self.num_spk,
            1)  # B*nmic, nspk, L, 3*win
        ...
        all_bf_output = all_bf_output.view(batch_size, nmic, self.num_spk, seq_length,
                                           self.window)  # B, nmic, nspk, L, win

        # reshape to utterance
        bf_signal = all_bf_output.view(batch_size * nmic * self.num_spk, -1, self.window * 2)
        bf_signal1 = bf_signal[:, :, :self.window].contiguous().view(batch_size * nmic * self.num_spk, 1, -1)[:, :,
                     self.stride:]
        bf_signal2 = bf_signal[:, :, self.window:].contiguous().view(batch_size * nmic * self.num_spk, 1, -1)[:, :,
                     :-self.stride]
